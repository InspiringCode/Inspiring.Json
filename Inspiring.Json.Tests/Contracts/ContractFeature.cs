    ...
    public class ContractFeature : Feature {
        ...
        internal void ContractTests(ContractRegistry reg, TestContractFactory factory, ContractTypeHierarchy h) {
            ...
            AND["the subcontracts include only types with contract attribute"] = () =>
                h.Subcontracts.Should().BeEquivalentTo(new[] {
            ...
            WHEN["the base class has also a discriminator value"] = () => { };
            ...
            WHEN["not specifying a discriminator value"] = () => { };
            ...
            WHEN["IsRootContract is set to true"] = () => reg.IsPolymorphic(typeof(BaseC), out h).Should().BeTrue();
            THEN["its discriminator value is set to the default value"] = () => h.DiscriminatorName.Should().Be("$type");
